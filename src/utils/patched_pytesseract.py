#!/usr/bin/env python
"""
Patched version of pytesseract with Windows space handling fix.

This is a patched version of madmaze/pytesseract that fixes the issue
with character whitelists containing spaces on Windows.

The fix: Changed line 330 from:
    cmd_args += shlex.split(config, posix=not_windows)
to:
    cmd_args += shlex.split(config, posix=True)

This ensures that spaces in character whitelists (like ' 0123456789')
are handled correctly on Windows by forcing POSIX-style parsing.

Original: https://github.com/madmaze/pytesseract
Issue: https://github.com/madmaze/pytesseract/issues/575
"""
...
def run_tesseract(
    ...
    if config:
        # PATCH: Force POSIX mode to properly handle spaces in character whitelists on Windows
        # Original: cmd_args += shlex.split(config, posix=not_windows)
        cmd_args += shlex.split(config, posix=True)
