...
def batch_image_to_strings(
    ...
    rows, final_lang = _run_tesseract_tsv(composite, whitelist, psm, lang)
...
def _run_tesseract_tsv(
    ...
    available_langs = []
    for requested_lang in lang.split("+"):
        if os.path.exists(os.path.join(tessdata_dir, f"{requested_lang}.traineddata")):
            available_langs.append(requested_lang)
    ...
    tsv = pytesseract.image_to_data(image, config=config, lang=final_lang)
    return _parse_tsv(tsv), final_lang
